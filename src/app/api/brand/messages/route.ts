...
export async function POST(req: Request) {
  ...
  const { data: th } = await sb.from('broker_threads').select('lounge_id').eq('id', b.threadId).maybeSingle();
  const loungeId = (th as { lounge_id: string } | null)?.lounge_id;
  if (loungeId) await notifyLoungeOwner(loungeId, s.brand.name, 'broker_message', body.slice(0, 100));
