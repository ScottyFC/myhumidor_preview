...
export async function GET() {
  ...
  try {
    ...
    const { data } = await sb.from('brand_wholesale_listings')
      .select('id, cigar_name, vitola, cigars_per_box, price_per_box_cents, boxes_available, moq_boxes, image_url, brand_id, brands!inner(name, slug, tier)')
      .eq('status', 'active').order('created_at', { ascending: false });
    type Row = { id: string; cigar_name: string; vitola: string | null; cigars_per_box: number; price_per_box_cents: number; boxes_available: number; moq_boxes: number; image_url: string | null; brand_id: string; brands: { name: string; slug: string; tier: string } };
    const byBrand = new Map<string, { brandId: string; name: string; slug: string; listings: unknown[] }>();
    for (const r of ((data ?? []) as unknown as Row[])) {
      if (r.brands?.tier !== 'premium') continue;
