...
the insert passes RLS. Both the client gate (`isVerifiedLoungeOperator`) and the
`catalog_cigars` insert policy required `lounges.verified = true` — but **nothing in
the app ever sets `verified`** (certification sets `certified` / `cert_tier`). So every
real lounge's submission failed the catalog insert, stayed `pending` in
`cigar_submissions`, and never reached search / the detail page / "recently added".

Fix — gate on **certified OR verified** on both sides:
- `isVerifiedLoungeOperator` now accepts `certified` or `verified` lounges.
- `phase83.sql` updates the `catalog_cigars` insert policy to match.

After phase83, a **certified** lounge owner submitting a cigar with full details
(brand, name, country, size) auto-publishes — it shows in search, on its detail page,
and in "recently added" (search already merges `searchCatalogCigarsRemote`; the detail
page already falls back to the DB). Non-certified lounges still go to the admin queue,
which promotes to the catalog on approval (that path already worked).

Notes: for a specific lounge to auto-publish it must be `certified = true` (or
verified) AND the owner must be linked in `lounge_members` (phase81). phase83 has an
optional commented backfill to mark all certified lounges verified too. Can't test
live RLS here.
## Everything needed to run on device (no migration)

Added `scripts/native-setup.mjs` + npm scripts and a full runbook
(`docs/MOBILE_SETUP.md`). The app is a Capacitor shell around the hosted site, so the
device build is: generate native projects → apply permissions → assets → run.

- `npm run cap:permissions` (= the script): idempotently injects ALL required
  permissions into the generated projects — iOS Info.plist (Camera, PhotoLibrary,
  PhotoLibraryAdd, FaceID, LocationWhenInUse) and AndroidManifest (CAMERA, fine/coarse
  location, POST_NOTIFICATIONS, READ_MEDIA_IMAGES, USE_BIOMETRIC + camera feature).
  Capacitor doesn't add usage strings itself; the missing iOS camera string is the #1
  cause of "scan does nothing" on device.
- New scripts: `cap:permissions`, `cap:assets` (icon/splash), `cap:setup`
  (permissions + sync). Existing `cap:add:*`, `cap:ios/android`, `cap:sync` retained.
- Runbook covers prerequisites, the one-time create/permission/asset steps, push setup
  (APNs capability + Firebase google-services.json), the bundled Cap-8 plugin list, and
  required hosting env vars.

Caveat: the `ios/` and `android/` projects are generated by `npx cap add` with the
native toolchains (and excluded from this zip) — I can't create or build them here.
The script + runbook get them device-ready in a few commands; push needs Apple/Firebase
account setup, and icons need source art. Real-device verification still required.
